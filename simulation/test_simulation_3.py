...
# ABOUT THIS SCRIPT:                                                          #
#                                                                             #
# - Takes the initial state files (ending in "_is") in set1, simulates a      #
#   scenario using the current decision rule, and saves it to set2.           #
#                                                                             #
###############################################################################
...
def main():
    ...
    while II < NUM_SIMULATIONS:
        ...
        # Load vehicle file
        v_list = g.load_pickled_data(
            f"{SET1_SAVE_LOC}{SIM_DATA_PKL_NAME}_{II}{IS_SUFF}"
        )

        # Generate the simulation
        print()
        is_cc = sim.generate_single_simulation(
            PLG_, SAVE_LOC=SAVE_LOC, II=str(II), save_initial_state=False, v_list=v_list
        )
        if is_cc:
            SAVE_SUFF = CC_SUFF
            cc_list.append(II)
