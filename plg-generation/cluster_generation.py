...
#                                                                             #
###############################################################################
def cluster_generation(PLG: PLG):
    start_nodes = []
    target_nodes = []
    ...
    # Start nodes
    kmeans_start = KMeans(
        n_clusters=NUM_START_CLUSTERS,
        init="k-means++",
        n_init=1,
        max_iter=num_kmeans_iterations,
    ).fit(start_node_coords)
    start_node_cluster_labels = kmeans_start.labels_

    # Target nodes
    kmeans_target = KMeans(
        n_clusters=NUM_TARGET_CLUSTERS,
        init="k-means++",
        n_init=1,
        max_iter=num_kmeans_iterations,
    ).fit(target_node_coords)
    ...
    for ii in closest_clusters_dict:
        ...
        # centre
        dist_to_centre_ii = np.linalg.norm(
            cluster_centre_coords - centre_ii, ord=2, axis=1
        )
